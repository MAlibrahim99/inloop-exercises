...
public class Deliverable extends ProjectItem{
    ...
    public Deliverable(String name, String details, double rate, long materialCost, double productionTime,
                       LocalDate date) {
        super(name, details, rate);
        if(materialCost < 0 || productionTime <= 0 ){
            throw new IllegalArgumentException();
        }
        if(date == null){ // check if there are other validation roles
